select 
  ...
  amr.AMR_RANK_CD,
  amr.QA_SCR,
  cast(amr.RCV_TIME as datetime) REG_TIME,
  amr.REG_USER_ID,
  amr.REG_DEPT_CD,
  amr.RCV_USER_ID,
  amr.RCV_DEPT_CD,
  cast(amr.RCV_TIME as datetime) RCV_TIME,
  amr.ILL_STA_CD,
  amr.NU_LVL_CD,
  amr.CUR_MAIN_DI_CD,
  amr.CUR_DEPT_CD,
  cast(amr.CUR_DEPT_INTO_TIME as datetime) CUR_DEPT_INTO_TIME,
  amr.CUR_WARD_CD,
  cast(amr.CUR_WARD_INTO_TIME as datetime) CUR_WARD_INTO_TIME,
  amr.CUR_GRP_CD,
  amr.CUR_BED_CD,
  amr.CUR_MNG_DCT_ID,
  amr.CUR_MOJ_DCT_ID,
  ...
  amr.CALL_BACK_F,
  amr.FLUP_VISIT_F,
  cast(amr.FNSH_TIME as datetime) FNSH_TIME,
  amr.FNSH_USER_ID,
  amr.FNSH_DEPT_CD,
  m.CUR_MOJ_DCT_ID,
  m.CUR_DRC_DCT_ID,
 -- cast(amr.TERM_QA_CMPL_TIME as datetime) TERM_QA_CMPL_TIME,
  --amr.TERM_QA_CMPL_USER_ID,
 --amr.TERM_QA_CMPL_DEPT_CD,
  '1' VIEW_F
  from VW_EN_ZY@dg4msql amr left join mr_amr m on amr.en_pk=m.en_pk
