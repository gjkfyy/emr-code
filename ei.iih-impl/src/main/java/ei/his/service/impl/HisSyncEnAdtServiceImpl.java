...
import ei.his.db.dao.CusHisIemrEnAdtEntityDao;
import ei.his.db.dao.HisIemrEnAdtEntityDao;
 ...
 * 从His中 同步 “转科记录”
 */
public class HisSyncEnAdtServiceImpl implements HisSyncEnAdtService {

	public static Logger log = LoggerFactory
			.getLogger(HisSyncEnAdtServiceImpl.class);

	@Autowired
	CusHisIemrEnAdtEntityDao cusHisIemrEnAdtEntityDao;

	@Autowired
	HisIemrEnAdtEntityDao IemrEnAdtEntityDao;
	 ...
	 */
	@Override
	public void SychronizeEncounterAdtFromHis(Map map) {
		List<IemrEnAdtEntity> list = cusHisIemrEnAdtEntityDao.selectAll();
		if (list != null) {
			int size = list.size();
			log.debug(" His_Iemr_En_Adt ,count = " + size);
			if (size > 0) {
