 ...
 *          </table>
 */
public class ScoreMrType extends ScoreCaculate implements ScoreDrop {
	...
	private static final Logger log = LoggerFactory.getLogger(ScoreMrType.class);
	...
	public double dropScore(double dropScoreValue) {
		...
		if (this.getMaxDropScore() <= this.dropTotalScore) {
			log.warn("病例文书:" + this.getCode() + " 所扣总分值已经超过病例文书最大可扣分值");
			log.warn("设置扣分值为病例文书最大可扣分值:" + this.getMaxDropScore());
