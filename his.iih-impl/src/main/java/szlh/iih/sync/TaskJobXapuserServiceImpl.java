...
public class TaskJobXapuserServiceImpl implements TaskJobXapUserService {
	...
	public String search() {
		...
		List<XapUserPo> ListXapUser = cusXapuserDao.selectAll(options);
...
//		                          + "]");
	...
	private String InsertSql(XapUserPo xapUserPo){
		String isSyncUserPwd = isSyncUserPwd();

		//StringBuffer xap_user_bf = new StringBuffer();
		xap_user_bf.delete(0, xap_user_bf.length());
		xap_user_bf.append(" insert into XAP_USER (");
		xap_user_bf.append("XAP_USER_PK , USER_ID,  EMP_ID , USER_STA_CD, SIGN_PIC, PWD, IEMR_PWD, NM, NM_SPELL, SEX_CD, EMAIL, JOB_TITLE_CD, MOBILE_NO, TEL_NO, ORG_CD, LOGIN_NM,DCT_NS_F,Del_F) values (");
		...
		if(xapUserPo.getPwd() != null && !xapUserPo.getPwd().equals(null)  && !xapUserPo.getPwd().equals("") && xapUserPo.getPwd() != "" ){
			//wangyanli add 20160523 如果为1，则表示要同步密码，否则，根据具体的密码值赋值
			if(isSyncUserPwd.equals("1")){ 
				xap_user_bf.append("'"+xapUserPo.getPwd()+"'");
			}else{				
				xap_user_bf.append("'"+ isSyncUserPwd +"'");
			}
		 }else{
			 if(isSyncUserPwd.equals("1")){
				 isSyncUserPwd = "MTIzNA==";
			 }
			 xap_user_bf.append("'"+ isSyncUserPwd +"'");
		 }
		xap_user_bf.append(",");
		...
		xap_user_bf.append(",");
		if(xapUserPo.getDctNsF() != null && !xapUserPo.getDctNsF().equals(null)){
			xap_user_bf.append("'"+xapUserPo.getDctNsF()+"'");
		}else{
			xap_user_bf.append("''");
		} 
		 xap_user_bf.append(",");
	...
	private String UpdateSql(XapUserPo xapUserPo){
		...
		//去掉图片签名字段的更新
		xap_user_bf.append(" USER_ID,  EMP_ID , USER_STA_CD, PWD, NM, NM_SPELL, SEX_CD, EMAIL, JOB_TITLE_CD, MOBILE_NO, TEL_NO, ORG_CD, LOGIN_NM)");
    ...
    //是否同步用户密码，规则为：如果值=1则需要同步，如果值!=1则具体不同的值为，需要赋值的密码值,如果参数没设置，则密码设为MTIzNA==
	private String isSyncUserPwd() {

		String syncUserPwd="MTIzNA==";
		Param[] emrCallHisParams=paramsService.getParam(SysFunctionCode.SYNC_USER_PWD).getDataList();
		for(int i=0; i<emrCallHisParams.length ; i++){
			Param param=emrCallHisParams[i];
			if(param.getKey().equals("SYNC_USER_PWD") && null!=param.getValue()){
				syncUserPwd=param.getValue();
				break;
			}
		}
