Ext.define('iih.mr.wr.nmr.action.BatchDocInitAction', {
	...
	*/
	execute: function(context) {
	    Ext.useShims=true;
	    var a=Ext.getCmp('nursingrecordsview');
    	...
    	this.getOwner().getBlock('bottom').opType='open';
    	var opType=this.getOwner().getBlock('bottom').opType;
    	if(opType!=undefined){
    		var rs = this.getOwner().getBlock('bottom');;   
    		if(opType!='readonly'){
...
//      var self=this;
    	...
    	var mrDocEditPage=Ext.getCmp('batchdoceditview');
    	...
    	if(medicalRecord.canEdit&&1==medicalRecord.canEdit){
     	...
     	var owner = this.getOwner().getBlock('bottom');
        var properties = owner.getLayer(Xap.ej.block.Layer.PROPERTIES);
    	if(medicalRecord.onlyXmlNeedToTransfer){
    		if(medicalRecord.filePk){
    			//如果不存在filePk,则该文件需要新建，保存先刷新宏元素，然后再合并xml
    			properties.setData({
                    mergeXmlDataToTemplate:true,
                    refreshMacro:true,
                    xmlFileData:medicalRecord.xmlFileData 
                });
    		}else{
    			//只是更新xml内容到odt中
    			properties.setData({
