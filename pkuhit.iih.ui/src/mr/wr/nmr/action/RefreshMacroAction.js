Ext.define('iih.mr.wr.nmr.action.RefreshMacroAction', {
...
//        Ext.Msg.alert("提示",'刷新宏元素成功!');
    ...
    refreshMacroElement:function(plugin,macroElementValue){
        ...
        var Fall=null;
        var Cathether=null;
        var mrSplFmValues = macroElementValue.mrSplFmValue;
        if(mrSplFmValues!=null&&mrSplFmValues.length>0)
        for (var i = 0; i < mrSplFmValues.length; i++) {
			var msf=mrSplFmValues[i];
			if(msf!=null&&msf.fmType==2){
				if(msf.fmName=='Braden'){
					Braden=msf.value;//Braden评估分
					continue;
				}else if(msf.fmName=='Barther'){
					Barther=msf.value;//Barther评估分
					continue;
				}else if(msf.fmName=='Fall'){
					Fall=msf.value;//Fall评估分
					continue;
				}else if(msf.fmName=='Catheter'){
					Cathether=msf.value;//Catheter评估分
				}
			}
		}
//        console.log(encounter);
        ...
        for ( var i = 0; i < allMacroIdsArray.length; i++) {
            ...
            case macroConstants.MR_ELEMENT_CD_36://就诊日期
                ...
                break;
            case macroConstants.MR_ELEMENT_CD_37://护士签名
                break;
            case macroConstants.MR_ELEMENT_CD_38://二级护士签名
                break;
            case macroConstants.MR_ELEMENT_CD_39://三级护士签名
                break;
            ...
            case macroConstants.MR_ELEMENT_CD_115://户口地址
                ...
                break;
            case macroConstants.MR_ELEMENT_CD_121://病人来源
            	macroValue = patient.reservedColumn1;
                break;
            case macroConstants.MR_ELEMENT_CD_117://Braden评估分
            	macroValue = Braden;
            	if(!Braden)
            		macroValue = "/";
                break;
            case macroConstants.MR_ELEMENT_CD_118://Barther评估分
            	macroValue = Barther;
            	if(!Barther)
            		macroValue = "/";
                break;
            case macroConstants.MR_ELEMENT_CD_119://Fall评估分
            	macroValue = Fall;
            	if(!Fall)
            		macroValue = "/";
                break;
            case macroConstants.MR_ELEMENT_CD_120://Catheter评估分
            	macroValue = Cathether;
            	if(!Cathether)
            		macroValue = "/";
                break;
            case macroConstants.MR_ELEMENT_CD_122://病人诊断
            	macroValue =  patient.reservedColumn2;
                break;
            case macroConstants.MR_ELEMENT_CD_123://护理级别
            	macroValue =  patient.reservedColumn3;
                break;
            default:
                if (unKnownMacros == '') {
            ...
            if(macroId!='MR.M.013.D.24'&&macroId!='MR.M.013.D.25'&&macroId!='MR.M.013.D.26'&&macroId!='MR.M.013.D.37'&&macroId!='MR.M.013.D.38'&&macroId!='MR.M.013.D.39'){
